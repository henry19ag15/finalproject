...
export const validateForm = (form) => {
  ...
  const expresiones = {
    ...
    password:
      /^(?=.*?[A-Z])(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?=(.*[\W]){1,})(?!.*\s).{8,}$/, // 6 a 14 digitos.
  ...
  if (!expresiones.email.test(form.email.trim())) {
    errors.displayError =
      "Debe ser un correo valido y solo puede contener letras, numeros, puntos, guiones y guion bajo";
  }

  if (!expresiones.password.test(form.password.trim())) {
    errors.displayError =
      "La contraseña debe tener mínimo ocho caracteres, al menos una letra mayúscula, un número y un carácter especial";
  }
  ...
  if (form.password.trim() !== form.repeatPassword.trim()) {
    errors.displayError =
      "La contraseña ingresada debe ser igal en los dos casilleros";
  }
